...
class PersonSearch(viewsets.GenericViewSet):
    ...
    def _list(self, request, include_inactive: bool = False):
        ...
        start = DateUtils.datetime_from_isoformat(
            self.request.GET.get("start", None))
        end = DateUtils.datetime_from_isoformat(
            self.request.GET.get("end", None))
        type = InsuranceTypeEnum.parse_type(
        ...
        if not group_group_admin_id:
            ...
            logger.debug(
                "Searching for members and non-members with term and group %s", group_group_admin_id)

        if start and end:
            logger.debug(
                "Searching for non-members who are already insured between %s and %s", start, end)
        ...
        logger.debug(
            f"Filtered inuits non-members: {len(inuits_non_members)} results"
        )
        if type:
            if inuits_non_members and start and end:
                ...
                if type in non_filtering_insurance_types:
                    logger.debug(
                        f"Not filtering inuits non-members for {non_filtering_insurance_types}"
                    )
                    pass
        ...
        logger.debug("InuitsNonMember: %s", ",".join(
            [person.last_name for person in inuits_non_members]))
        ...
        logger.debug("UNIQUE InuitsNonMember: %s", ",".join(
            [person.last_name for person in unique_non_members]))
