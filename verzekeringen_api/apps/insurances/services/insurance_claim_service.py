...
def generate_pdf(claim):
    owner: GroupAdminMember = group_admin_member_detail(active_user=claim.declarant,
                                                        group_admin_id=claim.declarant.group_admin_id)
    victim = claim.victim
    model = {
        ...
        if property['/T'] == '(Code_activiteit)':
            for activity_type in claim.activity_type:
                ...
                if activity_type == 'TRANSPORT':
                    property.update(PdfDict(AS=PdfName('061_Op_weg_naar_van_activiteit'),
                                            V=PdfName('061_Op_weg_naar_van_activiteit')))
                    property['/Kids'][2].update(PdfDict(AS=PdfName('061_Op_weg_naar_van_activiteit'),
                                                        V=PdfName('061_Op_weg_naar_van_activiteit')))
